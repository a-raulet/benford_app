    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel("Commentaires",
                 ...
                 tags$br(),
                 h5("De 1965 à 20007, les données par circonscriptions sont utilisées, mais l'analyse de Benford ne peut pas être appliquée : l'ordre de grandeur robuste est trop faible, et les distributions des données sont symétriques, suivant davantage une courbe normale."),
                 tags$br(),
                 h5("Les données de 2012 à 2022 (données de chaque bureau de votes regroupées par villes) remplissent les conditions (à part pour certains candidats), et deux tests sont effectués pour vérifier si les données sont conformes ou non avec la loi de Benford : la somme des carrés des différences de Kossovsky, et l'écart moyen absolu de Nigrini."),
                 tags$br(),
